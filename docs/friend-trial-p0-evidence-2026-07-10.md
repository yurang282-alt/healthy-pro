...
- Repository `social` code removes `friendCode` from internal friend/ranking summaries while preserving the current user's own code in their private profile response. Automated deployment first failed twice at `getCloudAPISignedHeader` with platform error `41002`; the user then deployed `social` manually in WeChat DevTools, and a subsequent read-only function list confirmed `social` online. The platform list does not expose a source hash, so this evidence relies on the user's successful-deployment report plus online presence.
